...
class DetectionLoss(nn.Module):
    def __init__(self, iou_threshold=None, neg_pos_ratio=None, use_focal_loss=True):
        ...
        # Add confidence margin loss parameters - NEW
        self.confidence_margin = 0.4  # Target margin between positive and negative samples
        self.use_margin_loss = True   # Enable confidence margin loss
    ...
    def forward(self, predictions, targets):
        ...
        total_margin_loss = torch.tensor(0., device=device)  # NEW: margin loss component
        num_pos = 0
        
        for i in range(batch_size):
            batch_loc_loss, batch_conf_loss, batch_margin_loss, batch_num_pos = self._compute_batch_loss(
            ...
            total_margin_loss += batch_margin_loss  # NEW: add margin loss
        ...
        total_margin_loss = total_margin_loss / num_pos  # NEW: normalize margin loss
        
        # Improved dynamic weighting for better convergence
        loc_weight = 3.0  # Heavy emphasis on localization
        conf_weight = 1.0  # Standard weight for confidence
        margin_weight = 0.5  # Weight for the margin component
        
        # Total loss with margin component
        total_loss = loc_weight * total_loc_loss + conf_weight * total_conf_loss
        if self.use_margin_loss:
            total_loss += margin_weight * total_margin_loss
    ...
    def _compute_batch_loss(self, bbox_pred, conf_pred, default_anchors, target, valid_pred_mask, is_val_mode):
        ...
        # Create targets for positive anchors with improved positive sample selection
        ...
        for gt_idx in range(num_gt):
            ...
            # Additionally, assign top-2 anchors for each ground truth for more stable training
            if ious.shape[0] > 2:  # Only if we have enough anchors
                gt_ious = ious[:, gt_idx]
                _, top_idx = gt_ious.topk(min(3, len(gt_ious)))
        ...
        margin_loss = torch.tensor(0., device=device)  # NEW: initialize margin loss
        
        if num_positive > 0:
            # Localization loss - using DIoU loss for better bounding box quality
            matched_gt_boxes = gt_boxes.squeeze(0)[best_gt_idx[positive_indices]]
            pred_boxes = bbox_pred[positive_indices]
            
            # Use DIoU loss for better localization
            ...
            # NEW: Calculate margin loss to enforce separation between positive and negative confidences
            if self.use_margin_loss and num_positive > 0:
                # Get all negative examples (non-positive)
                negative_mask = ~positive_mask
                if valid_pred_mask is not None:
                    negative_mask = negative_mask & valid_pred_mask
                
                negative_indices = torch.where(negative_mask)[0]
                
                if len(negative_indices) > 0:
                    # Sample negative examples (same as in conf loss)
                    ...
                    num_neg = min(num_positive * self.neg_pos_ratio, len(neg_indices))
                    num_neg = max(num_neg, 1)  # At least one negative
                    neg_indices = neg_indices[:num_neg]
                    
                    # Get confidence scores for positives and hard negatives
                    pos_conf = conf_pred[positive_indices]
                    neg_conf = conf_pred[neg_indices]
                    
                    # Calculate margin loss: encourage positive scores to be higher than negatives by margin
                    if len(neg_conf) > 0 and len(pos_conf) > 0:
                        pos_conf_mean = pos_conf.mean()
                        neg_conf_mean = neg_conf.mean()
                        
                        # Margin loss: max(0, margin - (pos_conf_mean - neg_conf_mean))
                        current_margin = pos_conf_mean - neg_conf_mean
                        margin_loss = torch.clamp(self.confidence_margin - current_margin, min=0)
        
        return loc_loss, conf_loss, margin_loss, num_positive
    ...
    def _compute_negative_loss(self, conf_pred, valid_pred_mask, is_val_mode):
        ...
        # Return with an additional zero tensor for the margin loss
        return torch.tensor(0., device=device), conf_loss, torch.tensor(0., device=device), 0
    ...
    def _compute_conf_loss(self, conf_pred, conf_target, positive_indices, valid_pred_mask, num_positive):
        ...
        # Apply hard negative mining with improved sampling
        _, neg_indices = neg_conf_loss.sort(descending=True)
        ...
        # Ensure minimum number of negative samples
        num_neg = max(num_neg, num_positive)
        neg_indices = neg_indices[:num_neg]
